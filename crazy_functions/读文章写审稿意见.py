...
def 解析Paper(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    for index, fp in enumerate(file_manifest):
        ...
        prefix = "接下来请你逐文件分析下面的论文文件，并编写审稿意见" if index==0 else ""
        i_say = prefix + f'请对下面的文章片段提出审稿意见，文件名是{os.path.relpath(fp, project_folder)}，文章内容是 ```{file_content}```'
        i_say_show_user = prefix + f'[{index}/{len(file_manifest)}] 请对下面的文章提出审稿意见: {os.path.abspath(fp)}'
    ...
    i_say = f'根据以上你自己的分析，作为文章的审稿人，用学术性语言提出审稿意见，（包括{all_file}）。'
...
@CatchException
def 读文章写审稿意见(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    history = []    # 清空历史，以免输入溢出
    import glob, os
